...
from the C<init> method just like L<Mojolicious> plugins can to other Mojo apps.
Here is a very simple example showing how to add a route:
...
This sort of plugin might be useful for adding a web UI for L<PlugAuth> for example.
Here is a very simple test for this plugin:
...
configuration for the test using L<YAML>'s C<DumpFile> function.  Finally we use
...
plugins to reload the authentiation and authorization database from flat files if
they have changed.  There is also a simple test module L<Test::PlugAuth::Plugin::Refresh>
