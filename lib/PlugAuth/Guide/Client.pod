...
This document contains some common recipies for using interacting as a client
...
=head2 From a L<Clustericious> service using L<Clustericious::Plugin::SimpleAuth>.

Any Clustericious service can be integrated with PlugAuth using the Clustericious
L<SimpleAuth|Clustericious::Plugin::SimpleAuth> plugin.  Here is an example Routes
module
...
For more details seee L<Clustericious::Plugin::SimpleAuth>.
...
There is a L<PAM module that will authenticate against a HTTP server using 
HTTP Basic authentication on git|https://github.com/beatgammit/pam-http>. It 
is licensed under the MIT License and is based on work by Kragen Sitaker.  I
had to install libpam0g-dev and libcurl4-openssl-dev in order for it to build.
