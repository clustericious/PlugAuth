...
# VERSION
# PODNAME: PlugAuth::Guide::ServerQuickstart
...
In your favorite text editor, create this configuration:

 ---
 % use File::HomeDir;  my $home = File::HomeDir->my_home;
 url: http://localhost:3000
 user_file: <%= $home %>/etc/plugauth/user.txt
 group_file: <%= $home %>/etc/plugauth/group.txt
 resource_file: <%= $home %>/etc/plugauth/resource.txt
 host_file: <%= $home %>/etc/plugauth/host.txt
...
out with they only need to be empty files:
 ...
 % touch ~/etc/plugauth/{user,group,resource,host}.txt
...
users, groups and permissions:
 ...
 --- ok
 % plugauthclient grant admin accounts /user
 --- ok
 % plugauthclient grant admin accounts /group
 --- ok
 % plugauthclient grant admin accounts /grant
 --- ok
...
I<simple_auth> configuration.  Your PlugAuth.conf should look like this:
...
htpasswd command and a simple text editor.  

=over 4

=item * create configuration

Do as above, but create your configuration initially with I<simple_auth> set in your 
PlugAuth.conf file:
...
Use htpasswd to create the admin user:
 ...
 % htpasswd -m ~/etc/plugauth/user.txt admin
 New password: 
...
Using your favorite text editor, add these lines to the ~/etc/plugauth/resource.txt file:
...
L<PlugAuth::Plugin::DBI::Auth>, L<PlugAuth::Plugin::AuthenSimple>
